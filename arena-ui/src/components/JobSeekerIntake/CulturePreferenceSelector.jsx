...
const CulturePreferenceSelector = ({ selectedPreferences, setSelectedPreferences }) => {
    const togglePreference = (e, preference) => {
      ...
      <PreferenceContainer>
        {culturePreferenceOptions.map((preference, index) => (
          <PreferenceButton
            ...
            onClick={(e) => togglePreference(e, preference)}
            disabled={selectedPreferences.length >= 3 && !selectedPreferences.includes(preference)}
          >
            {preference}
          </PreferenceButton>
        ))}
